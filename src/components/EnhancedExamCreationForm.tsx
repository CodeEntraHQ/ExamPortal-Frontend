...
export function EnhancedExamCreationForm({ 
  ...
  const handleSubmit = () => {
    if (validateStep(1) && validateStep(2) && validateStep(3) && validateStep(4)) {
      ...
      const calculatedTotalMarks = formData.questions.reduce((sum, q) => sum + q.points, 0);
      const finalFormData = {
        ...formData,
  ...
  const checkMicrophone = async () => {
    ...
    try {
      ...
      showNotification('Microphone access denied. This may affect proctoring features.', 'error');
    }
  };
    ...
    return (
      <QuestionManagement
        ...
        onQuestionsUpdate={(questions) => {
          ...
          if (totalMarks > 0) {
            handleInputChange('totalMarks', totalMarks);
          }
        }}
                    ...
                    <div>
                      ...
                      {formData.questions.length > 0 && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Will be calculated from questions: {formData.questions.reduce((sum, q) => sum + q.points, 0)} points
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="text-2xl font-bold">
                          {Math.round(formData.questions.reduce((sum, q) => sum + q.points, 0) / formData.questions.length)}
                        </div>
